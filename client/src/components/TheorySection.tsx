...
import bigBounceImage from "@/assets/images/big-bounce.avif";
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8 mb-16">
          {theorySteps.map((step, index) => (
            <div
              key={index}
              className="bg-background dark:bg-dark-bg rounded-xl shadow-lg overflow-hidden transform transition duration-300 hover:-translate-y-2 hover:shadow-xl border border-border"
            >
              <div className="p-1 bg-cosmic-gradient"></div>
              <div className="p-6">
                <div
                  className={`w-12 h-12 rounded-full bg-${step.colorClass}/10 dark:bg-${step.colorClass}/20 flex items-center justify-center mb-4`}
                >
                  <span
                    className={`text-${step.colorClass} font-display font-bold text-xl`}
                  >
                    {step.number}
                  </span>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {keyImplications.map((implication, index) => (
              <div key={index} className="flex">
                <div
                  className={`flex-shrink-0 h-12 w-12 flex items-center justify-center rounded-full bg-${implication.colorClass}/10 dark:bg-${implication.colorClass}/20 mr-4`}
                >
                  <i
                    className={`bx ${implication.icon} text-${implication.colorClass} text-2xl`}
                  ></i>
                </div>
                <div>
